...
class VentanaVentas(QtWidgets.QMainWindow):
    ...
    def buscarCliente(self):
        ...
        if self.txtCodigoCliente.text() == "":
            QtWidgets.QMessageBox.information(self, "Código Cliente", "Debe ingresar el código del  
                                                  cliente...!!!", QtWidgets.QMessageBox.Ok)
        else:
    ...
    def buscarProducto(self):
        ...
        if self.txtCodigoProducto.text() == "":
            QtWidgets.QMessageBox.information(self, "Código Producto", "Debe ingresar el código del
                                                  producto...!!!", QtWidgets.QMessageBox.Ok)
        else:
    ...
    def imprimirFactura(self):
        try:
            if self.txtNumeroDocumento.text() == "":
                QtWidgets.QMessageBox.information(self, "Imprimir Factura", "No se ha realizado ninguna 
                                                       venta...!!!", QtWidgets.QMessageBox.Ok)    
            else:                
    ...
    def agregar(self):
        try:
            if int(self.txtCantidad.text()) > int(self.txtStock.text()):
                ...
                lista.append((self.obtenerItem(), self.obtenerCodigo(), self.obtenerDescripcion(),                                   self.obtenerPrecio(), self.obtenerCantidad(),
            ...
            QtWidgets.QMessageBox.information(self, "Agregar Detalle", "No se ha completado los detalles del 
                                                    producto...!!!", QtWidgets.QMessageBox.Ok) 
            self.item = self.item - 1
    ...
    def generarVenta(self):
        if self.tblDetalle.rowCount() == 0:
              QtWidgets.QMessageBox.information(self, "Venta", "No se ha agregado ningún  
                                                detalle...!!!", QtWidgets.QMessageBox.Ok)
        else: 
            ...
            if aFac.buscarFactura(numeroFactura) == -1:
                ...
                QtWidgets.QMessageBox.information(self, "Venta", "El número de factura ingesado ya 
                                                       existe...!!!", QtWidgets.QMessageBox.Ok)
